...
#in my case the 'myData.AVG' object was saved from a previous R analysis and is simply a customized dataframe of gene expression data produced using dplyr
...
#using 'myData.AVG' object created from dplyr mutate command
load("myData.AVG")

shinyServer(function(input, output) {
  tooltip <- function(data, ...) {
    paste0("<b>", data$symbols, "</b>")
  }
  ...
  vis <- reactive({
    ...
    #passing 'myData.AVG' to ggvis to make a graph 
    myData.AVG %>% 
      ggvis(x= xvar, y= yvar, key := ~symbols) %>% 
      add_tooltip(tooltip)
  }) 
  ...
  output$view <- DT::renderDataTable(myData.AVG, server = TRUE)
})
